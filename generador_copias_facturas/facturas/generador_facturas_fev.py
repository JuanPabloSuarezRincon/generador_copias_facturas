...
import os
from generador_copias_facturas.datos import generador_datos_fev  # Importación corregida
...
def generar_facturas(num_documento_obligado_entry, tipo_nota_entry, num_factura_personalizada_entry, num_facturas_inicio_entry, num_facturas_fin_entry, ruta_entry, edad_min_entry, edad_max_entry, barra_progreso, ventana):
    try:
        ...
        for i in range(num_facturas_inicio, num_facturas_fin + 1):
            factura = generador_datos_fev.generar_factura(i, num_documento_obligado, tipo_nota, num_factura_personalizada, edad_min, edad_max)  # Llamada corregida
...
def main():
    ...
    # Encabezado
    encabezado_label = ttk.Label(ventana, text="Factura Electrónica de Venta (fev)", font=("Arial", 12, "bold"))
