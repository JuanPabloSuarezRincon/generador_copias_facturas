#generador_facturas_rs.py
import json
import tkinter as tk
from tkinter import filedialog, messagebox, ttk
import os
from generador_copias_facturas.datos import generador_datos_rs
...
def generar_facturas(num_documento_obligado_entry, num_fev_inicio_entry, num_fev_fin_entry, num_facturas_inicio_entry, num_facturas_fin_entry, ruta_entry, edad_min_entry, edad_max_entry, barra_progreso, ventana):
    try:
        num_documento_obligado = num_documento_obligado_entry.get()
        num_fev_inicio = int(num_fev_inicio_entry.get())
        num_fev_fin = int(num_fev_fin_entry.get())
        ...
        fev_counter = num_fev_inicio
        for i in range(num_facturas_inicio, num_facturas_fin + 1):
            num_factura_personalizada = f"FEV{fev_counter:03d}"
            factura = generador_datos_rs.generar_factura(i, num_documento_obligado, num_factura_personalizada, edad_min, edad_max)
...
def main():
    ...
    # Rango de números FEV
    frame_num_fev = ttk.LabelFrame(ventana, text="Rango de Números FEV")
    frame_num_fev.pack(padx=10, pady=10, fill="x")

    num_fev_inicio_label = ttk.Label(frame_num_fev, text="Número inicial FEV:")
    num_fev_inicio_label.pack(side=tk.LEFT, padx=5, pady=5)

    num_fev_inicio_entry = ttk.Entry(frame_num_fev)
    num_fev_inicio_entry.pack(side=tk.LEFT, padx=5, pady=5)

    num_fev_fin_label = ttk.Label(frame_num_fev, text="Número final FEV:")
    num_fev_fin_label.pack(side=tk.LEFT, padx=5, pady=5)

    num_fev_fin_entry = ttk.Entry(frame_num_fev)
    num_fev_fin_entry.pack(side=tk.LEFT, padx=5, pady=5)
    ...
    num_facturas_fin_entry.pack(side=tk.LEFT, padx=5),
    # Ruta de guardado
    ...
    generar_button = ttk.Button(ventana, text="Generar Facturas", command=lambda: generar_facturas(num_documento_obligado_entry, num_fev_inicio_entry, num_fev_fin_entry, num_facturas_inicio_entry, num_facturas_fin_entry, ruta_entry, edad_min_entry, edad_max_entry, barra_progreso, ventana))
